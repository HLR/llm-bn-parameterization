...
from classification.utils.prompts import build_NB_FullDist_prior_prompt, build_NB_FullDist_dependant_prompt

def run_NB_FullDist(chat_model,result_file_name,data_file_folder):
    ...
    for name, tr, td, te, target, desc, features, feature_values, target_values, df_merged in iter_train_test(-1,0.8):
        ...
        if os.path.exists(model_path):
            ...
            prior_prompt, prior_fixing_parser = build_NB_FullDist_prior_prompt(chat_model)
            prior_messages = prior_prompt.format_messages(dataset_name=name, dataset_desc=desc, target_var=target, target_values=target_values)
            prior_response = chat_model.invoke(prior_messages)
            prior_result = prior_fixing_parser.invoke(prior_response.content)
            priors = prior_result.probabilities
            priors = {k.lower(): v for k, v in priors.items()}
            print(f"Prior probabilities: {priors}")

            column_prompt, single_fix_parser = build_NB_FullDist_dependant_prompt(chat_model)
            cpts = {}
            for feature in tqdm(features, desc="Querying LLM for CPTs"):
                ...
                for t_val in target_values:
                    ...
                    dist = single_fix_parser.invoke(raw.content).probabilities
                ...
                col_order = [col_distributions[t][fv] for fv in feature_values[feature] for t in target_values]
